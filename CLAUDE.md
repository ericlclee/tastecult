...
**Current stage:** Backend MVP in progress — **backend first**; the user designs the frontends themselves, so `/apps/web` is a deliberately blank Next.js shell wired to the API and mobile hasn't started. **Mobile is the main product, but the web app comes first as the surface for demos and design iteration** — so the API must serve browser clients well from the start, not only mobile. **The web demo behaves like the iPhone app, and iPhone comes before Android** (decided with the user): design web features as the iPhone app would work, and where the platforms differ, match iPhone first and treat Android as a later adaptation. For example, the planned restaurant suggestions from a photo's location: iPhone photos usually carry GPS, which a native app can read with full photo library access, while Android hides it by default (the `ACCESS_MEDIA_LOCATION` permission, the system photo picker, Google Play's limits on full photo access). Browsers on either platform often strip it, so on the web fall back to the phone's current location (`restaurant.nearby`), then name search. Photos must still be stripped of location before upload, since the bucket is public. Done: monorepo scaffold + CI, Prisma schema and initial migration, shared tier/ranking logic, FHRS London restaurant import (~51k restaurants), TasteAtlas dish catalogue import (9,980 dishes, 42 cuisines). Also built: the tRPC API (`/apps/api`) with its first public endpoints — `health`, `cuisine.list`, `restaurant.search`/`nearby`/`byId`, `dish.search`/`byId` — plus the typed client (`/packages/api-client`) and the blank web shell. Deployed: the Supabase database (London; migrated and loaded) and both apps as separate Vercel projects. The API is public at `https://api-two-pi-32.vercel.app` (London, `lhr1`); the web app is still behind Vercel Authentication. Also built, and verified end to end against the real Supabase project (a genuine sign-in token checked via JWKS, a photo uploaded to the public `dish-photos` bucket, a log saved and listed): magic-link sign-in, profiles (username), and the logging loop — now **one visit at a time, with any number of dishes on it** (decided with the user): restaurant (nearby or search) and date once, then a dish (search, or request a missing one), tier and optional menu name / cuisine / note per dish, plus photos (camera or upload, resized in the browser, uploaded straight to Supabase Storage) that belong to the visit and can each point at one dish on it — plus a "my logs" list, an explore page (search restaurants and dishes, then see everyone's logs against them), public profiles with following, follower and following lists, and a feed of visits from people you follow. The feed, `/me` and public profiles show one entry per visit with its dishes inside; restaurant and dish pages stay one row per dish. The web screens for it are deliberately unstyled apart from what the user directs. UI work has started, step by step and led by the user: **the web app is laid out as a phone screen** (decided with the user) — one centred column at most 430px wide, set with Tailwind classes in `apps/web/app/layout.tsx`. Style with Tailwind utility classes rather than new CSS files. In development, `apps/web/app/dev-inspector.tsx` labels whatever you hover with the components it's inside (e.g. `LogFeed › LogSocial › button "Comments (0)"`) — the user refers to UI by these labels, so keep component names meaningful. Alt+click copies a label; Alt+Shift+I toggles it. Also in development, `apps/web/app/design-panel.tsx` (left of the screen, outside the app column) switches design settings — currently the text font — by setting a data attribute on `<html>` that `globals.css` maps to a CSS variable; fonts are loaded in `apps/web/app/fonts.ts`. Add future settings (brand colours, spacing) the same way. Neither tool is rendered in production. There's no home page (decided with the user): `/` redirects to `/explore`, and one menu in the root layout (`apps/web/app/site-nav.tsx`) appears on every page, so pages don't carry their own nav links. The `dish-photos` bucket exists (public, 5 MB limit, JPEG only). Next: set the Supabase variables on both Vercel projects, try the flow in a browser, then an admin flow for requested dishes.
...
| Styling / cross-platform UI | **Tailwind CSS** (web) → **NativeWind** (mobile)                                                | Decided with the user (replacing the earlier Tamagui plan), who wanted the most popular, best-documented option. Tailwind v4 in the Next.js web app now; NativeWind (Tailwind for React Native) when mobile starts, reusing the same class names and theme. NativeWind isn't used inside Next.js because its docs require an existing React Native Web setup and support only client pages. Screens are built per platform; design tokens and class vocabulary are shared.                                                                     |
...
/apps
  ...
  /api          — tRPC API (src/routers), served by one Next.js route handler at app/api/trpc/[trpc];
                  shared log/visit selects, views and helpers live in src/logs.ts so the visit
                  and rating routers can't drift apart
/packages
  ...
  /ui           — Shared design tokens (Tailwind theme) and UI primitives       [not started — frontend deferred]
...
- **Visit** — one trip to one restaurant on one day, holding every dish eaten on it (decided with the user). It carries the user, restaurant, `visitedAt` and an optional overall `note`; the dishes are its `Rating` rows. A person can have two visits to the same restaurant on the same date (lunch and dinner), so there's deliberately no unique key on that. Visits are the write path — `visit.create`/`update`/`delete` are the only mutations, and a single-dish log is just a visit with one dish
- **VisitPhoto** — photos belong to the visit and may each also point at one dish on it (`ratingId`), so a photo of the table and a photo of the ramen live side by side (decided with the user). `ratingId` is set to null rather than cascaded when a dish is removed from a visit, so a table photo survives. Ordered by `position`; `path` is unique
- **Rating** — one dish logged on a visit: a required menu item (so restaurant and dish are required), `tier`; optional `cuisineId` and `note`. `userId` and `visitedAt` are denormalised from the visit so every list, index and ordering clause works without a join — keep them in sync whenever a visit changes. The logged cuisine is the logger's call for that visit and any cuisine is valid; the UI suggests the dish's cuisines, with an "Other" option that unlocks the rest. Every visit is new rows. Owners **edit and delete whole visits**: `visit.update` replaces the visit, keeping the id of every dish sent with one (so reactions and comments stay with it), adding dishes sent without one and deleting the logs left out. Photos are matched by path the same way — ones left out are removed from the visit and from storage. A menu item left with no logs after an edit or delete is deleted. Deleting a visit takes every dish on it, with their reactions, comments and photos. Replaced or deleted photos are removed from Storage *after* the database change is saved, and a failed removal is only logged (`console.error`), because a leftover file is harmless but a failed edit isn't. **Logs are public, but the full list needs an account** (decided with the user): on restaurant and dish pages anyone sees the summary and the latest `PUBLIC_LOG_PREVIEW` (3) logs, and only signed-in people can page through the rest. The API enforces this, not just the web app. Logs of a requested dish are visible only to its requester. **Stats use only each user's latest log per menu item** (`latestLogPerUser` in shared-types) so repeat visits don't stack votes. Restaurant and dish pages stay one row per dish (`rating.forRestaurant`/`forDish`), because those pages are about that dish or restaurant; the feed, your own logs and public profiles group by visit (`visit.feed`/`mine`/`forUser`)
...
**What a visit captures:** restaurant (required), date (required, defaults to today), note (optional), photos (optional, each optionally of one dish on the visit), and one or more dishes. **What each dish captures:** the dish (required), tier (required), dish alias (optional), cuisine (optional — the dish's cuisines are offered, "Other" unlocks all 42), note (optional).
...
# Demo data — LOCAL database only: 25 mock people, ~600 logs on ~385 visits, follows, placeholder photos
...
- **`Rating.userId` and `Rating.visitedAt` are denormalised from its `Visit`.** They exist so lists, indexes and ordering clauses need no join, which is why the visit routers set them on every write. Anything that changes a visit's owner, restaurant or date must update its ratings in the same transaction — `visit.update` does. A new write path must too.
- **Photos hang off the visit, not the dish.** `Rating` has no `photoPath`; a photo is a `VisitPhoto` row with a unique `path`, an optional `ratingId` naming one dish on the visit, and a `position`. The API takes and returns a photo's dish as `dishIndex` — an index into the visit's own `dishes` — because a new visit's dishes have no ids yet.
...
- **Demo data is recognisable by design.** Mock people's ids start with `d0000000-0000-4000-8000-` and their placeholder photos live under `demo/` in the `dish-photos` bucket. `scripts/seed-demo` only removes rows and files matching those, refuses to run unless `DATABASE_URL` points at localhost, and never touches real accounts' own logs — it only adds follows between them and demo people. The plan is deterministic (fixed seed), so re-seeding gives the same people, venues and ratings. Logs sharing a person, restaurant and date become one visit, and the plan deliberately puts more than one dish on about 40% of visits, so the seed exercises multi-dish visits.
- **Hand-edited migration SQL:** the `pg_trgm` extension and the `Rating_tier_check` CHECK constraint are in the init migration SQL because `schema.prisma` can't express them. Keep them intact if a future migration recreates those objects. `20260920120000_visits` is hand-written too: it groups every existing log into a visit by (user, restaurant, date) and moves `Rating.photoPath` into `VisitPhoto` before dropping the column, so its statements have to stay in that order.
