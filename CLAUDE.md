...
| Database                    | Postgres + Prisma ORM                                      | Relational integrity between users, restaurants, dishes, ratings; add a vector DB later only if/when recommendation search needs it. **Hosted on Supabase Postgres** (decided with the user; London region planned), next to Supabase Auth and Storage — not yet provisioned                                                                                                                               |
